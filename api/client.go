...
import (
	...
	goctlAPI "github.com/khulnasoft-lab/go-goctl/v2/pkg/api"
)

const (
	accept          = "Accept"
	authorization   = "Authorization"
	cacheTTL        = "X-GOCTL-CACHE-TTL"
...
func (c Client) GraphQL(hostname string, query string, variables map[string]interface{}, data interface{}) error {
	...
	gqlClient, err := goctlAPI.NewGraphQLClient(opts)
	if err != nil {
		return err
	}
...
func (c Client) REST(hostname string, method string, p string, body io.Reader, data interface{}) error {
	opts := clientOptions(hostname, c.http.Transport)
	restClient, err := goctlAPI.NewRESTClient(opts)
	if err != nil {
		return err
	}
...
func HandleHTTPError(resp *http.Response) error {
	return handleResponse(goctlAPI.HandleHTTPError(resp))
}

// handleResponse takes a goctlAPI.HTTPError or goctlAPI.GraphQLError and converts it into an
...
func handleResponse(err error) error {
	...
	var restErr *goctlAPI.HTTPError
	...
	var gqlErr *goctlAPI.GraphQLError
...
func generateScopesSuggestion(statusCode int, endpointNeedsScopes, tokenHasScopes, hostname string) string {
	...
	for _, s := range strings.Split(endpointNeedsScopes, ",") {
		...
		return fmt.Sprintf(
			"This API operation needs the %[1]q scope. To request it, run:  goctl auth refresh -h %[2]s -s %[1]s",
			s,
...
func clientOptions(hostname string, transport http.RoundTripper) goctlAPI.ClientOptions {
	// AuthToken, and Headers are being handled by transport,
	// so let go-goctl know that it does not need to resolve them.
	opts := goctlAPI.ClientOptions{
		AuthToken: "none",
