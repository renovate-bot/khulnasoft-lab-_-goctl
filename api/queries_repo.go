	...
	if result.Repository == nil {
		return nil, GraphQLError{
			GraphQLError: &goctlAPI.GraphQLError{
				Errors: []goctlAPI.GraphQLErrorItem{{
					Type:    "NOT_FOUND",
...
func ForkRepo(client *Client, repo ghrepo.Interface, org, newName string, defaultBranchOnly bool) (*Repository, error) {
	...
	// The GitHub API will happily return a HTTP 200 when attempting to fork own repo even thougoctl no forking
...
func RepoFindForks(client *Client, repo ghrepo.Interface, limit int) ([]*Repository, error) {
	...
	for _, r := range result.Repository.Forks.Nodes {
		// we check ViewerCanPush, even thougoctl we expect it to always be true per
...
func RepoExists(client *Client, repo ghrepo.Interface) (bool, error) {
	...
	case 404:
		return false, nil
	default:
		return false, goctlAPI.HandleHTTPError(resp)
	}
}
