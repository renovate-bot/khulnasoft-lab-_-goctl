...
//     - GOCTL_VERSION: determined from source by default
//     - GOCTL_OAUTH_CLIENT_ID
//     - GOCTL_OAUTH_CLIENT_SECRET
...
var tasks = map[string]func(string) error{
	"bin/goctl": func(exe string) error {
		...
		ldflags = fmt.Sprintf("-X github.com/khulnasoft-lab/goctl/v2/internal/build.Version=%s %s", version(), ldflags)
		ldflags = fmt.Sprintf("-X github.com/khulnasoft-lab/goctl/v2/internal/build.Date=%s %s", date(), ldflags)
		if oauthSecret := os.Getenv("GOCTL_OAUTH_CLIENT_SECRET"); oauthSecret != "" {
			ldflags = fmt.Sprintf("-X github.com/khulnasoft-lab/goctl/v2/internal/authflow.oauthClientSecret=%s %s", oauthSecret, ldflags)
			ldflags = fmt.Sprintf("-X github.com/khulnasoft-lab/goctl/v2/internal/authflow.oauthClientID=%s %s", os.Getenv("GOCTL_OAUTH_CLIENT_ID"), ldflags)
		}
...
func version() string {
	if versionEnv := os.Getenv("GOCTL_VERSION"); versionEnv != "" {
		return versionEnv
	}
