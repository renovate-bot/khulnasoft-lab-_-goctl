...
func NewCmdSet(f *cmdutil.Factory, runF func(*SetOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ goctl secret set MYSECRET

			# Read secret value from an environment variable
			$ goctl secret set MYSECRET --body "$ENV_VALUE"

			# Read secret value from a file
			$ goctl secret set MYSECRET < myfile.txt

			# Set secret for a deployment environment in the current repository
			$ goctl secret set MYSECRET --env myenvironment

			# Set organization-level secret visible to both public and private repositories
			$ goctl secret set MYSECRET --org myOrg --visibility all

			# Set organization-level secret visible to specific repositories
			$ goctl secret set MYSECRET --org myOrg --repos repo1,repo2,repo3

			# Set user-level secret for Codespaces
			$ goctl secret set MYSECRET --user

			# Set repository-level secret for Dependabot
			$ goctl secret set MYSECRET --app dependabot

			# Set multiple secrets imported from the ".env" file
			$ goctl secret set -f .env

			# Set multiple secrets from stdin
			$ goctl secret set -f - < myfile.txt
		`),
