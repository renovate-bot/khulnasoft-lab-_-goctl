...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/secret/shared"
...
func TestNewCmdSet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cli:  `-u -bs -r"monalisa/coolRepo,khulnasoft-lab/goctl,github/hub" cool_secret`,
			wants: SetOptions{
				SecretName:      "cool_secret",
				Visibility:      shared.Selected,
				RepositoryNames: []string{"monalisa/coolRepo", "khulnasoft-lab/goctl", "github/hub"},
				Body:            "s",
			},
		},
		{
		...
		{
			name: "Codespaces org",
			cli:  `random_secret -ocoolOrg -b"random value" -vselected -r"coolRepo,khulnasoft-lab/goctl" -aCodespaces`,
			wants: SetOptions{
				SecretName:      "random_secret",
				Visibility:      shared.Selected,
				RepositoryNames: []string{"coolRepo", "khulnasoft-lab/goctl"},
				Body:            "random value",
				OrgName:         "coolOrg",
	...
	tests := []struct {
		...
		},
		{
			...
			opts: &SetOptions{
				UserSecrets:     true,
				Visibility:      shared.Selected,
				RepositoryNames: []string{"khulnasoft-lab/goctl", "github/hub"},
			},
