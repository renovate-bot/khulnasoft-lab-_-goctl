...
func NewCmdAdd(f *cmdutil.Factory, runF func(*AddOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			Note that any key added by goctl will be associated with the current authentication token.
		...
		Example: heredoc.Doc(`
			...
			$ ssh-keygen -t ed25519 -C "my description" -N "" -f ~/.ssh/goctl-test
			$ goctl repo deploy-key add ~/.ssh/goctl-test.pub
		`),
