...
func TestNewCmdSync(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			output: SyncOptions{
				DestArg: "khulnasoft-lab/goctl",
			},
		},
		{
			name:  "source repo",
			tty:   true,
			input: "--source khulnasoft-lab/goctl",
			output: SyncOptions{
				SrcArg: "khulnasoft-lab/goctl",
			},
		},
		{
			name:  "branch",
...
func Test_SyncRun(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "Upstream commits contain workflow changes, which require the `workflow` scope to merge. To request it, run: goctl auth refresh -s workflow",
		},
	}
	for _, tt := range tests {
