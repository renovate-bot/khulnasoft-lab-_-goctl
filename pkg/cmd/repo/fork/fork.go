...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/repo/shared"
	...
	if inParent {
		...
		if cloneDesired {
			...
			if connectedToTerminal {
				...
				fmt.Fprintf(stderr, "%s Repository %s set as the default repository. To learn more about the default repository, run: goctl repo set-default --help\n", cs.WarningIcon(), cs.Bold(ghrepo.FullName(repoToFork)))
			}
		}
	}
