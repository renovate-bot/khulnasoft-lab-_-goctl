...
func TestRepoFork(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// goctl repo fork --remote --remote-name origin | cat
			tty: true,
			opts: &ForkOptions{
				Remote:     true,
				RemoteName: defaultRemoteName,
		...
		},
		{
			...
			execStubs: func(cs *run.CommandStubber) {
				...
				cs.Register(`git -C REPO config --add remote.upstream.goctl-resolved base`, 0, "")
			},
			wantErrOut: "✓ Created fork someone/REPO\n✓ Cloned fork\n! Repository OWNER/REPO set as the default repository. To learn more about the default repository, run: goctl repo set-default --help\n",
		},
		{
