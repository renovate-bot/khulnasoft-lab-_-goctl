...
func TestDefaultRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			gitStubs: func(cs *run.CommandStubber) {
				cs.Register(`git config --unset remote.origin.goctl-resolved`, 0, "")
			},
		...
		},
		{
			...
			wantStdout: "no default repository has been set; use `goctl repo set-default` to select one\n",
		},
		{
		...
		},
		{
			...
			gitStubs: func(cs *run.CommandStubber) {
				cs.Register(`git config --add remote.upstream.goctl-resolved base`, 0, "")
			},
		...
		},
		{
			...
			gitStubs: func(cs *run.CommandStubber) {
				cs.Register(`git config --add remote.upstream.goctl-resolved OWNER2/REPO2`, 0, "")
			},
			wantStdout: "✓ Set OWNER2/REPO2 as the default repository for the current directory\n",
		},
		{
