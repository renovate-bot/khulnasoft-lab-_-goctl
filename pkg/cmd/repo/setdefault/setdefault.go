...
func NewCmdSetDefault(f *cmdutil.Factory, runF func(*SetDefaultOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ goctl repo set-default

			Set a repository explicitly:
			$ goctl repo set-default owner/repo

			View the current default repository:
			$ goctl repo set-default --view
...
func setDefaultRun(opts *SetDefaultOptions) error {
	...
	if opts.ViewMode {
		if currentDefaultRepo != nil {
			...
			fmt.Fprintln(opts.IO.Out, "no default repository has been set; use `goctl repo set-default` to select one")
		}
		return nil
	}
