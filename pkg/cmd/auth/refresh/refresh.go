...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/internal/authflow"
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/set"
...
func NewCmdRefresh(f *cmdutil.Factory, runF func(*RefreshOptions) error) *cobra.Command {
	...
	cmd.Flags().StringSliceVarP(&opts.Scopes, "scopes", "s", nil, "Additional authentication scopes for goctl to have")
...
func refreshRun(opts *RefreshOptions) error {
	...
	if len(candidates) == 0 {
		return fmt.Errorf("not logged in to any hosts. Use 'goctl auth login' to authenticate with a host")
	}
	...
	if hostname == "" {
		...
		if !found {
			return fmt.Errorf("not logged in to %s. use 'goctl auth login' to authenticate with this host", hostname)
		}
	}
