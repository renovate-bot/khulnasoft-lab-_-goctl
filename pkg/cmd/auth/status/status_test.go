...
func Test_statusRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantOut: heredoc.Doc(`
				joel.miller
				  ✓ Logged in to joel.miller as tess (GOCTL_CONFIG_DIR/hosts.yml)
		...
		},
		{
			...
			wantErrOut: heredoc.Doc(`
				joel.miller
				  X joel.miller: the token in GOCTL_CONFIG_DIR/hosts.yml is missing required scope 'read:org'
				  - To request missing scopes, run: goctl auth refresh -h joel.miller
				
				github.com
				  ✓ Logged in to github.com as tess (GOCTL_CONFIG_DIR/hosts.yml)
		...
		},
		{
			...
			wantErrOut: heredoc.Doc(`
				joel.miller
				  ...
				  - The joel.miller token in GOCTL_CONFIG_DIR/hosts.yml is invalid.
				  - To re-authenticate, run: goctl auth login -h joel.miller
				  - To forget about this host, run: goctl auth logout -h joel.miller
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			output := strings.ReplaceAll(stdout.String(), config.ConfigDir()+string(filepath.Separator), "GOCTL_CONFIG_DIR/")
			errorOutput := strings.ReplaceAll(stderr.String(), config.ConfigDir()+string(filepath.Separator), "GOCTL_CONFIG_DIR/")
