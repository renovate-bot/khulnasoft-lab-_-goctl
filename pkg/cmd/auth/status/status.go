...
func NewCmdStatus(f *cmdutil.Factory, runF func(*StatusOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`Verifies and displays information about your authentication state.
			...
			This command will test your authentication state for each GitHub host that goctl knows about and
...
func statusRun(opts *StatusOptions) error {
	...
	for _, hostname := range hostnames {
		...
		if tokenSource == "oauth_token" {
			// The go-goctl function TokenForHost returns this value as source for tokens read from the
		...
		if err != nil {
			...
			if tokenIsWriteable {
				addMsg("- To re-authenticate, run: %s %s",
					cs.Bold("goctl auth login -h"), cs.Bold(hostname))
				addMsg("- To forget about this host, run: %s %s",
					cs.Bold("goctl auth logout -h"), cs.Bold(hostname))
			}
			failed = true
			continue
		}
