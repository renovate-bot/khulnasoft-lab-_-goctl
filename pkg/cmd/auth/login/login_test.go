...
func Test_NewCmdLogin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("GOCTL_HOST", tt.defaultHost)
	...
	tests := []struct {
		...
		},
		{
			...
			cfgStubs: func(c *config.ConfigMock) {
				...
				authCfg.SetToken("value_from_env", "GOCTL_TOKEN")
			...
			wantStderr: heredoc.Doc(`
				The value of the GOCTL_TOKEN environment variable is being used for authentication.
		...
		},
		{
			...
			cfgStubs: func(c *config.ConfigMock) {
				authCfg := c.Authentication()
				authCfg.SetToken("value_from_env", "GOCTL_ENTERPRISE_TOKEN")
				c.AuthenticationFunc = func() *config.AuthConfig {
					return authCfg
				}
			},
			wantErr: "SilentError",
			wantStderr: heredoc.Doc(`
				The value of the GOCTL_ENTERPRISE_TOKEN environment variable is being used for authentication.
				To have GitHub CLI store credentials instead, first clear the value from the environment.
			`),
		},
		{
