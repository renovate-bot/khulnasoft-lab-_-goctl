...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/auth/shared"
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmdutil"
	"github.com/khulnasoft-lab/goctl/v2/pkg/iostreams"
	ghAuth "github.com/khulnasoft-lab/go-goctl/v2/pkg/auth"
...
func NewCmdLogin(f *cmdutil.Factory, runF func(*LoginOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			If a credential store is not found or there is an issue using it goctl will fallback to writing the token to a plain text file.
			See %[1]sgoctl auth status%[1]s for its stored location.

			Alternatively, use %[1]s--with-token%[1]s to pass in a token on standard input.
			The minimum required scopes for the token are: "repo", "read:org".

			Alternatively, goctl will use the authentication token found in environment variables.
			This method is most suitable for "headless" use of goctl such as in automation. See
			%[1]sgoctl help environment%[1]s for more info.

			To use goctl in GitHub Actions, add %[1]sGOCTL_TOKEN: ${{ github.token }}%[1]s to "env".
		`, "`"),
		Example: heredoc.Doc(`
			...
			$ goctl auth login --with-token < mytoken.txt

			# authenticate with a specific GitHub instance
			$ goctl auth login --hostname enterprise.internal
		`),
...
func loginRun(opts *LoginOptions) error {
	...
	// The go-goctl Config object currently does not support case-insensitive lookups for host names,
	// so normalize the host name case here before performing any lookups with it or persisting it.
	// https://github.com/khulnasoft-lab/go-goctl/pull/105
