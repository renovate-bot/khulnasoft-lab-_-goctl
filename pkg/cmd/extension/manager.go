...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/findsh"
...
func (m *Manager) Dispatch(args []string, stdin io.Reader, stdout, stderr io.Writer) (bool, error) {
	...
	if ext.IsBinary() || runtime.GOOS != "windows" {
		...
		// Dispatch all extension calls througoctl the `sh` interpreter to support executable files with a
...
func (m *Manager) list(includeMetadata bool) ([]*Extension, error) {
	...
	for _, f := range entries {
		if !strings.HasPrefix(f.Name(), "goctl-") {
			continue
		}
		if f.IsDir() {
...
func (m *Manager) installBin(repo ghrepo.Interface, target string) error {
	...
	// if an arm64 binary is unavailable, fall back to amd64 if it can be executed througoctl Rosetta 2
	...
	if asset == nil {
		return fmt.Errorf(
			"%[1]s unsupported for %[2]s. Open an issue: `goctl issue create -R %[3]s/%[1]s -t'Support %[2]s'`",
...
func (m *Manager) Remove(name string) error {
	targetDir := filepath.Join(m.installDir(), "goctl-"+name)
