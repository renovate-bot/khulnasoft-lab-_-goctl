...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/internal/browser"
	"github.com/khulnasoft-lab/goctl/v2/internal/config"
	"github.com/khulnasoft-lab/goctl/v2/internal/ghrepo"
	"github.com/khulnasoft-lab/goctl/v2/internal/prompter"
...
func TestNewCmdExtension(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			isTTY:      true,
			wantStdout: "Showing 4 of 4 extensions\n\n   REPO                    DESCRIPTION\n✓  vilmibm/goctl-screensaver  terminal animations\n   cli/goctl-cool             it's just cool ok\n   samcoe/goctl-triage        helps with triage\n✓  github/goctl-gei           something something enterprise\n",
		},
		{
			...
			wantStdout: "installed\tvilmibm/goctl-screensaver\tterminal animations\n\tcli/goctl-cool\tit's just cool ok\n\tsamcoe/goctl-triage\thelps with triage\ninstalled\tgithub/goctl-gei\tsomething something enterprise\n",
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				values := url.Values{
					"page":     []string{"1"},
					"per_page": []string{"30"},
					"q":        []string{"screen topic:goctl-extension"},
				}
		...
		},
		{
			...
			wantStdout: "\tvilmibm/goctl-screensaver\tterminal animations\n",
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				values := url.Values{
					"page":     []string{"1"},
					"per_page": []string{"30"},
					"q":        []string{"license:GPLv3 topic:goctl-extension user:jillvalentine"},
				}
				results := searchResults(1)
				reg.Register(
					httpmock.QueryMatcher("GET", "search/repositories", values),
		...
		},
		{
			...
			browseStubs: func(b *browser.Stub) func(*testing.T) {
				return func(t *testing.T) {
					b.Verify(t, "https://github.com/search?q=topic%3Agoctl-extension&type=repositories")
				}
			},
		},
		{
			name: "install an extension",
			args: []string{"install", "owner/goctl-some-ext"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					return []extensions.Extension{}
				}
				...
				return func(t *testing.T) {
					...
					assert.Equal(t, "goctl-some-ext", installCalls[0].InterfaceMoqParam.RepoName())
					listCalls := em.ListCalls()
					assert.Equal(t, 1, len(listCalls))
				}
			},
		},
		{
			...
			args: []string{"install", "owner/goctl-existing-ext"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					e := &Extension{path: "owner2/goctl-existing-ext", owner: "owner2"}
		...
		},
		{
			...
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					e := &Extension{path: "owner/goctl-existing-ext", owner: "owner"}
					return []extensions.Extension{e}
				}
				return func(t *testing.T) {
					calls := em.ListCalls()
			...
			wantStderr: "! Extension owner/goctl-existing-ext is already installed\n",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "X Could not find extension 'owner/goctl-some-ext' on host github.com",
		},
		{
		...
		{
			name: "upgrade an extension goctl-prefix",
			args: []string{"upgrade", "goctl-hello"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.UpgradeFunc = func(name string, force bool) error {
					return nil
				}
		...
		},
		{
			...
			args: []string{"upgrade", "monalisa/goctl-hello"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.UpgradeFunc = func(name string, force bool) error {
					return nil
				}
		...
		{
			name: "remove extension goctl-prefix",
			args: []string{"remove", "goctl-hello"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.RemoveFunc = func(name string) error {
					return nil
				}
		...
		},
		{
			...
			args: []string{"remove", "monalisa/goctl-hello"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.RemoveFunc = func(name string) error {
					return nil
				}
		...
		},
		{
			...
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					ex1 := &Extension{path: "cli/goctl-test", url: "https://github.com/cli/goctl-test", currentVersion: "1"}
					ex2 := &Extension{path: "cli/goctl-test2", url: "https://github.com/cli/goctl-test2", currentVersion: "1"}
			...
			wantStdout: "goctl test\tcli/goctl-test\t1\ngoctl test2\tcli/goctl-test2\t1\n",
		},
		{
			...
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				...
				return func(t *testing.T) {
					...
					assert.Equal(t, 1, len(calls))
					assert.Equal(t, "goctl-test", calls[0].Name)
				}
			},
			isTTY: true,
			prompterStubs: func(pm *prompter.PrompterMock) {
			...
			wantStdout: heredoc.Doc(`
				✓ Created directory goctl-test
				✓ Initialized git repository
				✓ Made initial commit
				✓ Set up extension scaffolding

				goctl-test is ready for development!

				Next Steps
				- run 'cd goctl-test; goctl extension install .; goctl test' to see your new extension in action
		...
		},
		{
			...
			wantStdout: heredoc.Doc(`
				...
				✓ Built goctl-test binary

				goctl-test is ready for development!

				Next Steps
				- run 'cd goctl-test; goctl extension install .; goctl test' to see your new extension in action
				- run 'go build && goctl test' to see changes in your code as you develop
				- run 'goctl repo create' to share your extension with others
		...
		},
		{
			...
			wantStdout: heredoc.Doc(`
				...
				Next Steps
				- run 'cd goctl-test; goctl extension install .' to install your extension locally
				- fill in script/build.sh with your compilation script for automated builds
				- compile a goctl-test binary locally and run 'goctl test' to see changes
				- run 'goctl repo create' to share your extension with others
		...
		},
		{
			...
			args: []string{"install", "owner/goctl-hello", "--force"},
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					return []extensions.Extension{}
				}
				...
				return func(t *testing.T) {
					...
					installCalls := em.InstallCalls()
					assert.Equal(t, 1, len(installCalls))
					assert.Equal(t, "goctl-hello", installCalls[0].InterfaceMoqParam.RepoName())
				}
			},
			isTTY:      true,
			wantStdout: "✓ Installed extension owner/goctl-hello\n",
		},
		{
			...
			managerStubs: func(em *extensions.ExtensionManagerMock) func(*testing.T) {
				em.ListFunc = func() []extensions.Extension {
					return []extensions.Extension{
						&Extension{path: "owner/goctl-hello", owner: "owner"},
					}
				}
				em.InstallFunc = func(_ ghrepo.Interface, _ string) error {
					return nil
...
func Test_checkValidExtension(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantError: "extension repository name must start with `goctl-`",
		},
		{
