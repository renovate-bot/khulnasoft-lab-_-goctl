...
func newTestManager(dir string, client *http.Client, gitClient gitClient, ios *iostreams.IOStreams) *Manager {
	return &Manager{
		...
		newCommand: func(exe string, args ...string) *exec.Cmd {
			...
			cmd.Env = []string{"GOCTL_WANT_HELPER_PROCESS=1"}
			return cmd
		},
...
func TestManager_List(t *testing.T) {
	tempDir := t.TempDir()
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-hello", "goctl-hello")))
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-two", "goctl-two")))

	assert.NoError(t, stubBinaryExtension(
		filepath.Join(tempDir, "extensions", "goctl-bin-ext"),
		binManifest{
			Owner: "owner",
			Name:  "goctl-bin-ext",
			Host:  "example.com",
	...
	dirOne := filepath.Join(tempDir, "extensions", "goctl-hello")
	dirTwo := filepath.Join(tempDir, "extensions", "goctl-two")
...
func TestManager_list_includeMetadata(t *testing.T) {
	...
	reg.Register(
		httpmock.REST("GET", "api/v3/repos/owner/goctl-bin-ext/releases/latest"),
		httpmock.JSONResponse(
			release{
				Tag: "v1.0.2",
				Assets: []releaseAsset{
					{
						Name:   "goctl-bin-ext-windows-amd64",
	...
	assert.Equal(t, "https://example.com/owner/goctl-bin-ext", exts[0].URL())
}

func TestManager_Dispatch(t *testing.T) {
	tempDir := t.TempDir()
	extDir := filepath.Join(tempDir, "extensions", "goctl-hello")
	extPath := filepath.Join(tempDir, "extensions", "goctl-hello", "goctl-hello")
...
func TestManager_Dispatch_binary(t *testing.T) {
	tempDir := t.TempDir()
	extPath := filepath.Join(tempDir, "extensions", "goctl-hello")
	exePath := filepath.Join(extPath, "goctl-hello")
	bm := binManifest{
		Owner: "owner",
		Name:  "goctl-hello",
		Host:  "github.com",
...
func TestManager_Remove(t *testing.T) {
	...
	assert.Equal(t, "goctl-two", items[0].Name())
}
...
func TestManager_UpgradeExtensions(t *testing.T) {
	...
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-hello", "goctl-hello")))
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-two", "goctl-two")))
	assert.NoError(t, stubLocalExtension(tempDir, filepath.Join(tempDir, "extensions", "goctl-local", "goctl-local")))
	ios, _, stdout, stderr := iostreams.Test()
	gc, gcOne, gcTwo := &mockGitClient{}, &mockGitClient{}, &mockGitClient{}
...
func TestManager_UpgradeExtension_GitExtension(t *testing.T) {
	tempDir := t.TempDir()
	extensionDir := filepath.Join(tempDir, "extensions", "goctl-remote")
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-remote", "goctl-remote")))
	ios, _, stdout, stderr := iostreams.Test()
	gc, gcOne := &mockGitClient{}, &mockGitClient{}
...
func TestManager_UpgradeExtension_GitExtension_DryRun(t *testing.T) {
	tempDir := t.TempDir()
	extDir := filepath.Join(tempDir, "extensions", "goctl-remote")
	assert.NoError(t, stubExtension(filepath.Join(tempDir, "extensions", "goctl-remote", "goctl-remote")))
	ios, _, stdout, stderr := iostreams.Test()
	gc, gcOne := &mockGitClient{}, &mockGitClient{}
...
func TestManager_MigrateToBinaryExtension(t *testing.T) {
	...
	rs.Register(`git -C.*?goctl-remote remote -v`, 0, "origin  git@github.com:owner/goctl-remote.git (fetch)\norigin  git@github.com:owner/goctl-remote.git (push)")
	rs.Register(`git -C.*?goctl-remote config --get-regexp \^.*`, 0, "remote.origin.goctl-resolve base")

	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-remote/releases/latest"),
		httpmock.JSONResponse(
			release{
				Tag: "v1.0.2",
				Assets: []releaseAsset{
					{
						Name:   "goctl-remote-windows-amd64.exe",
						APIURL: "/release/cool",
					},
				},
			}))
	...
	manifest, err := os.ReadFile(filepath.Join(tempDir, "extensions/goctl-remote", manifestName))
	assert.NoError(t, err)
	...
	assert.Equal(t, binManifest{
		...
		Host:  "github.com",
		Tag:   "v1.0.2",
		Path:  filepath.Join(tempDir, "extensions/goctl-remote/goctl-remote.exe"),
	}, bm)

	fakeBin, err := os.ReadFile(filepath.Join(tempDir, "extensions/goctl-remote/goctl-remote.exe"))
	assert.NoError(t, err)
...
func TestManager_UpgradeExtension_BinaryExtension(t *testing.T) {
	...
	reg.Register(
		...
		httpmock.JSONResponse(
			release{
				...
				Assets: []releaseAsset{
					{
						Name:   "goctl-bin-ext-windows-amd64.exe",
						APIURL: "https://example.com/release/cool2",
					},
				},
			}))
	...
	manifest, err := os.ReadFile(filepath.Join(tempDir, "extensions/goctl-bin-ext", manifestName))
	assert.NoError(t, err)
	...
	assert.Equal(t, binManifest{
		...
		Host:  "example.com",
		Tag:   "v1.0.2",
		Path:  filepath.Join(tempDir, "extensions/goctl-bin-ext/goctl-bin-ext.exe"),
	}, bm)

	fakeBin, err := os.ReadFile(filepath.Join(tempDir, "extensions/goctl-bin-ext/goctl-bin-ext.exe"))
	assert.NoError(t, err)
	assert.Equal(t, "FAKE UPGRADED BINARY", string(fakeBin))
...
func TestManager_UpgradeExtension_GitExtension_Pinned(t *testing.T) {
	tempDir := t.TempDir()
	extDir := filepath.Join(tempDir, "extensions", "goctl-remote")
	assert.NoError(t, stubPinnedExtension(filepath.Join(extDir, "goctl-remote"), "abcd1234"))
...
func TestManager_Install_git(t *testing.T) {
	...
	extensionDir := filepath.Join(tempDir, "extensions", "goctl-some-ext")
	gc := &mockGitClient{}
	gc.On("Clone", "https://github.com/owner/goctl-some-ext.git", []string{extensionDir}).Return("", nil).Once()

	m := newTestManager(tempDir, &client, gc, ios)

	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-some-ext/releases/latest"),
		httpmock.JSONResponse(
			release{
				Assets: []releaseAsset{
					{
	...
	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-some-ext/contents/goctl-some-ext"),
		httpmock.StringResponse("script"))

	repo := ghrepo.New("owner", "goctl-some-ext")
...
func TestManager_Install_git_pinned(t *testing.T) {
	...
	extensionDir := filepath.Join(tempDir, "extensions", "goctl-cool-ext")
	gc, gcOne := &mockGitClient{}, &mockGitClient{}
	gc.On("ForRepo", extensionDir).Return(gcOne).Once()
	gc.On("Clone", "https://github.com/owner/goctl-cool-ext.git", []string{extensionDir}).Return("", nil).Once()
	gcOne.On("CheckoutBranch", "abcd1234").Return(nil).Once()

	m := newTestManager(tempDir, &client, gc, ios)

	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-cool-ext/releases/latest"),
		httpmock.JSONResponse(
			release{
				Assets: []releaseAsset{
					{
	...
	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-cool-ext/commits/some-ref"),
		httpmock.StringResponse("abcd1234"))
	reg.Register(
		httpmock.REST("GET", "repos/owner/goctl-cool-ext/contents/goctl-cool-ext"),
		httpmock.StringResponse("script"))

	_ = os.MkdirAll(filepath.Join(m.installDir(), "goctl-cool-ext"), 0700)
	repo := ghrepo.New("owner", "goctl-cool-ext")
...
func TestManager_Install_binary_pinned(t *testing.T) {
	repo := ghrepo.NewWithHost("owner", "goctl-bin-ext", "example.com")
	...
	reg.Register(
		httpmock.REST("GET", "api/v3/repos/owner/goctl-bin-ext/releases/tags/v1.6.3-pre"),
		httpmock.JSONResponse(
			release{
				Tag: "v1.6.3-pre",
				Assets: []releaseAsset{
...
func TestManager_Install_binary_unsupported(t *testing.T) {
	...
	reg.Register(
		...
		httpmock.JSONResponse(
			release{
				Assets: []releaseAsset{
					{
						Name:   "goctl-bin-ext-linux-amd64",
						APIURL: "https://example.com/release/cool",
					},
				},
			}))
	...
	err := m.Install(repo, "")
	assert.EqualError(t, err, "goctl-bin-ext unsupported for windows-amd64. Open an issue: `goctl issue create -R owner/goctl-bin-ext -t'Support windows-amd64'`")
...
func TestManager_repo_not_found(t *testing.T) {
	...
	reg.Register(
		httpmock.REST("GET", "api/v3/repos/owner/goctl-bin-ext"),
		httpmock.StatusStringResponse(404, `{}`))
...
func TestManager_Create(t *testing.T) {
	chdirTemp(t)
	err := os.MkdirAll("goctl-test", 0755)
	assert.NoError(t, err)

	ios, _, stdout, stderr := iostreams.Test()

	gc, gcOne := &mockGitClient{}, &mockGitClient{}
	gc.On("ForRepo", "goctl-test").Return(gcOne).Once()
	gc.On("CommandOutput", []string{"init", "--quiet", "goctl-test"}).Return("", nil).Once()
	gcOne.On("CommandOutput", []string{"add", "goctl-test", "--chmod=+x"}).Return("", nil).Once()
	gcOne.On("CommandOutput", []string{"commit", "-m", "initial commit"}).Return("", nil).Once()

	m := newTestManager(".", nil, gc, ios)

	err = m.Create("goctl-test", extensions.GitTemplateType)
	assert.NoError(t, err)
	files, err := os.ReadDir("goctl-test")
	assert.NoError(t, err)
	assert.Equal(t, []string{"goctl-test"}, fileNames(files))
...
func TestManager_Create_go_binary(t *testing.T) {
	...
	err = m.Create("goctl-test", extensions.GoBinTemplateType)
	...
	gitignore, err := os.ReadFile(filepath.Join("goctl-test", ".gitignore"))
	require.NoError(t, err)
	assert.Equal(t, heredoc.Doc(`
		/goctl-test
		/goctl-test.exe
	...
	files, err = os.ReadDir(filepath.Join("goctl-test", ".github", "workflows"))
	require.NoError(t, err)
	assert.Equal(t, []string{"release.yml"}, fileNames(files))

	assert.Equal(t, heredoc.Doc(`
		[go mod init github.com/jillv/goctl-test]
		[go mod tidy]
		[go build]
	`), stdout.String())
	assert.Equal(t, "", stderr.String())
...
func TestManager_Create_other_binary(t *testing.T) {
	...
	err = m.Create("goctl-test", extensions.OtherBinTemplateType)
	assert.NoError(t, err)
	...
	files, err = os.ReadDir(filepath.Join("goctl-test", "script"))
	assert.NoError(t, err)
