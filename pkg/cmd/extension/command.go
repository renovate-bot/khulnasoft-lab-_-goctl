...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/api"
	"github.com/khulnasoft-lab/goctl/v2/git"
	"github.com/khulnasoft-lab/goctl/v2/internal/ghrepo"
	"github.com/khulnasoft-lab/goctl/v2/internal/tableprinter"
	"github.com/khulnasoft-lab/goctl/v2/internal/text"
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/extension/browse"
...
func NewCmdExtension(f *cmdutil.Factory) *cobra.Command {
	...
	extCmd := cobra.Command{
		Use:   "extension",
		Short: "Manage goctl extensions",
		Long: heredoc.Docf(`
			GitHub CLI extensions are repositories that provide additional goctl commands.

			The name of the extension repository must start with "goctl-" and it must contain an
			executable of the same name. All arguments passed to the %[1]sgoctl <extname>%[1]s invocation
			will be forwarded to the %[1]sgoctl-<extname>%[1]s executable of the extension.

			An extension cannot override any of the core goctl commands. If an extension name conflicts
			with a core goctl command you can use %[1]sgoctl extension exec <extname>%[1]s.

			See the list of available extensions at <https://github.com/topics/goctl-extension>.
		`, "`"),
	...
	extCmd.AddCommand(
		func() *cobra.Command {
			...
			cmd := &cobra.Command{
				...
				Long: heredoc.Doc(`
					...
					This command behaves similarly to 'goctl search repos' but does not
					support as many search qualifiers. For a finer grained search of
					extensions, try using:

						goctl search repos --topic "goctl-extension"

					and adding qualifiers as needed. See 'goctl help search repos' to learn
				...
				RunE: func(cmd *cobra.Command, args []string) error {
					...
					for _, repo := range result.Items {
						if !strings.HasPrefix(repo.Name, "goctl-") {
							continue
						}
		...
		&cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				for _, c := range cmds {
					...
					t.AddField(fmt.Sprintf("goctl %s", c.Name()))
					t.AddField(repo)
		...
		func() *cobra.Command {
			...
			cmd := &cobra.Command{
				...
				Short: "Install a goctl extension from a repository",
				Long: heredoc.Doc(`
				...
				`),
				Example: heredoc.Doc(`
					$ goctl extension install owner/goctl-extension
					$ goctl extension install https://git.example.com/owner/goctl-extension
					$ goctl extension install .
				`),
		...
		func() *cobra.Command {
			...
			cmd := &cobra.Command{
				...
				Long: heredoc.Doc(`
					...
					interface for browsing, adding, and removing goctl extensions. A terminal
					...
					readers or higoctl zoom.

					For a more traditional way to discover extensions, see:

						goctl ext search

					along with goctl ext install, goctl ext remove, and goctl repo view.
				`),
				Args: cobra.NoArgs,
				RunE: func(cmd *cobra.Command, args []string) error {
					if !io.CanPrompt() {
		...
		&cobra.Command{
			...
			Long: heredoc.Doc(`
				...
				"owner/goctl-extension", you should pass "extension". You can use this command when
				the short name conflicts with a core goctl command.

				All arguments after the extension name will be forwarded to the executable
				of the extension.
			`),
			Example: heredoc.Doc(`
				# execute a label extension instead of the core goctl label command
				$ goctl extension exec label
			`),
		...
		func() *cobra.Command {
			...
			cmd := &cobra.Command{
				...
				Example: heredoc.Doc(`
					# Use interactively
					goctl extension create

					# Create a script-based extension
					goctl extension create foobar

					# Create a Go extension
					goctl extension create --precompiled=go foobar

					# Create a non-Go precompiled extension
					goctl extension create --precompiled=other foobar
				`),
				...
				RunE: func(cmd *cobra.Command, args []string) error {
					...
					if strings.HasPrefix(extName, "goctl-") {
						fullName = extName
						extName = extName[3:]
					} else {
						fullName = "goctl-" + extName
					}
					...
					steps := fmt.Sprintf(
						"- run 'cd %[1]s; goctl extension install .; goctl %[2]s' to see your new extension in action",
						fullName, extName)
					...
					if tmplType == extensions.GoBinTemplateType {
						...
						steps = heredoc.Docf(`
						- run 'cd %[1]s; goctl extension install .; goctl %[2]s' to see your new extension in action
						- run 'go build && goctl %[2]s' to see changes in your code as you develop`, fullName, extName)
					} else if tmplType == extensions.OtherBinTemplateType {
						steps = heredoc.Docf(`
						- run 'cd %[1]s; goctl extension install .' to install your extension locally
						- fill in script/build.sh with your compilation script for automated builds
						- compile a %[1]s binary locally and run 'goctl %[2]s' to see changes`, fullName, extName)
					}
					...
					out := heredoc.Docf(`
						...
						%[4]s
						%[5]s
						- run 'goctl repo create' to share your extension with others
...
func checkValidExtension(rootCmd *cobra.Command, m extensions.ExtensionManager, extName, extOwner string) (extensions.Extension, error) {
	if !strings.HasPrefix(extName, "goctl-") {
		return nil, errors.New("extension repository name must start with `goctl-`")
	}

	commandName := strings.TrimPrefix(extName, "goctl-")
...
func normalizeExtensionSelector(n string) string {
	...
	return strings.TrimPrefix(n, "goctl-")
}
