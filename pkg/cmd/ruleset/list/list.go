...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/ruleset/shared"
...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			Your access token must have the admin:org scope to use the --org flag, which can be granted by running "goctl auth refresh -s admin:org".
		`),
		Example: heredoc.Doc(`
			...
			$ goctl ruleset list --repo owner/repo --parents

			# List rulesets in an organization
			$ goctl ruleset list --org org-name
		`),
		Aliases: []string{"ls"},
