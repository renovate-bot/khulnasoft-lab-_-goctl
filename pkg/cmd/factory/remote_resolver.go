...
func (rr *remoteResolver) Resolver() func() (context.Remotes, error) {
	...
	return func() (context.Remotes, error) {
		...
		// For environment default host (GOCTL_HOST) do not fallback to cachedRemotes if none match
		if src != "default" {
		...
		if len(cachedRemotes) == 0 {
			if isHostEnv(src) {
				...
				return nil, errors.New("set the GOCTL_HOST environment variable to specify which GitHub host to use")
			}
			return nil, errors.New("none of the git remotes configured for this repository point to a known GitHub host. To tell goctl about a new GitHub host, please use `goctl auth login`")
		}
