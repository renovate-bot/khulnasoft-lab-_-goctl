			...
			rr := &remoteResolver{
				...
				getConfig: func() (config.Config, error) {
					...
					cfg.AuthenticationFunc = func() *config.AuthConfig {
						...
						if tt.override != "" {
							authCfg.SetDefaultHost(tt.override, "GOCTL_HOST")
						}
						return authCfg
					}
					return cfg, nil
...
func Test_OverrideBaseRepo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.envOverride != "" {
				t.Setenv("GOCTL_REPO", tt.envOverride)
			}
			f := New("1")
			rr := &remoteResolver{
				readRemotes: func() (git.RemoteSet, error) {
...
func Test_ioStreams_pager(t *testing.T) {
	tests := []struct {
		...
		{
			name: "GOCTL_PAGER and PAGER set",
			env: map[string]string{
				"GOCTL_PAGER": "GOCTL_PAGER",
				"PAGER":    "PAGER",
			},
			wantPager: "GOCTL_PAGER",
		},
		{
			name: "GOCTL_PAGER and config pager set",
			env: map[string]string{
				"GOCTL_PAGER": "GOCTL_PAGER",
			},
			config:    pagerConfig(),
		...
		{
			name: "GOCTL_PAGER set to blank string",
			env: map[string]string{
				"GOCTL_PAGER": "",
				"PAGER":    "PAGER",
			},
...
func Test_ioStreams_prompt(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "prompt disabled via GOCTL_PROMPT_DISABLED env var",
			env:            map[string]string{"GOCTL_PROMPT_DISABLED": "1"},
			promptDisabled: true,
		},
	}
	for _, tt := range tests {
