...
func Test_magicFieldValue(t *testing.T) {
	f, err := os.CreateTemp(t.TempDir(), "goctl-test")
	if err != nil {
		t.Fatal(err)
	}
	defer f.Close()
  ...
  - name: Send greeting
	...
	tests := []struct {
		...
		},
		{
			...
			httpStubs: stubs,
			wantOut:   "✓ Created workflow_dispatch event for workflow.yml at trunk\n\nTo see runs for this workflow, try: goctl run list --workflow=workflow.yml\n",
		},
		{
		...
		},
		{
			...
			wantOut: "✓ Created workflow_dispatch event for minimal.yml at trunk\n\nTo see runs for this workflow, try: goctl run list --workflow=minimal.yml\n",
		},
		{
			name: "prompt",
			tty:  true,
