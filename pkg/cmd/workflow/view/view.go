...
import (
	...
	runShared "github.com/khulnasoft-lab/goctl/v2/pkg/cmd/run/shared"
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/markdown"
...
func viewWorkflowInfo(opts *ViewOptions, client *api.Client, repo ghrepo.Interface, workflow *shared.Workflow) error {
	...
	// Footer
	if wr.TotalCount != 0 {
		fmt.Fprintf(out, "To see more runs for this workflow, try: goctl run list --workflow %s\n", filename)
	}
	fmt.Fprintf(out, "To see the YAML for this workflow, try: goctl workflow view %s --yaml\n", filename)
	return nil
}
