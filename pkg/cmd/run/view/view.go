...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/run/shared"
...
func NewCmdView(f *cmdutil.Factory, runF func(*ViewOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			# View a specific run
			$ goctl run view 12345

			# View a specific run with specific attempt number
			$ goctl run view 12345 --attempt 3

			# View a specific job within a run
			$ goctl run view --job 456789

			# View the full log for a specific job
			$ goctl run view --log --job 456789

			# Exit non-zero if a run failed
			$ goctl run view 0451 --exit-status && echo "run pending or passed"
		`),
...
func runView(opts *ViewOptions) error {
	...
	if selectedJob == nil {
		...
		if shared.IsFailureState(run.Conclusion) {
			fmt.Fprintf(out, "To see what failed, try: goctl run view %d --log-failed\n", run.ID)
		} else if len(jobs) == 1 {
			fmt.Fprintf(out, "For more information about the job, try: goctl run view --job=%d\n", jobs[0].ID)
		} else {
			fmt.Fprintf(out, "For more information about a job, try: goctl run view --job=<job-id>\n")
		}
		...
		if shared.IsFailureState(selectedJob.Conclusion) {
			fmt.Fprintf(out, "To see the logs for the failed steps, try: goctl run view --log-failed --job=%d\n", selectedJob.ID)
		} else {
			fmt.Fprintf(out, "To see the full job log, try: goctl run view --log --job=%d\n", selectedJob.ID)
		}
		fmt.Fprintf(out, cs.Gray("View this run on GitHub: %s\n"), run.URL)
...
func getRunLog(cache runLogCache, httpClient *http.Client, repo ghrepo.Interface, run *shared.Run, attempt uint64) (*zip.ReadCloser, error) {
	...
	filepath := filepath.Join(os.TempDir(), "goctl-cli-cache", filename)
...
func logFilenameRegexp(job shared.Job, step shared.Step) *regexp.Regexp {
	// As described in https://github.com/khulnasoft-lab/goctl/issues/5011#issuecomment-1570713070, there are a number of steps
...
//	    └── 2_somestepname.txt
//
// It iterates througoctl the list of jobs and tries to find the matching
