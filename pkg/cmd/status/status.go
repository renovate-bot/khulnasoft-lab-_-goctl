...
func NewCmdStatus(f *cmdutil.Factory, runF func(*StatusOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		`),
		Example: heredoc.Doc(`
			$ goctl status -e khulnasoft-lab/goctl -e khulnasoft-lab/go-goctl # Exclude multiple repositories
			$ goctl status -o cli # Limit results to a single organization
		`),
...
type StatusItem struct {
	...
	Identifier string // eg khulnasoft-lab/goctl#1234 or just 1234
...
func (s *StatusGetter) LoadNotifications() error {
	...
	// testing to really get enougoctl mentions. I would love to be able to just
...
func (s *StatusGetter) LoadSearchResults() error {
	...
	if err != nil {
		...
		if errors.As(err, &gqlErrResponse) {
			gqlErrors := make([]goctlAPI.GraphQLErrorItem, 0, len(gqlErrResponse.Errors))
			...
			if len(gqlErrors) == 0 {
				...
				err = api.GraphQLError{
					GraphQLError: &goctlAPI.GraphQLError{
						Errors: gqlErrors,
					},
				}
			}
