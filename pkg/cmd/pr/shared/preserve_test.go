...
func Test_PreserveInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErrLine:      `X operation failed. To restore: goctl issue create --recover .*testfile.*`,
			err:              true,
		...
		},
		{
			...
			wantErrLine:      `X operation failed. To restore: goctl pr create --recover .*testfile.*`,
			err:              true,
			wantPreservation: true,
		},
	}
