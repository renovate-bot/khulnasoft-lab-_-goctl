...
import (
	...
	ghContext "github.com/khulnasoft-lab/goctl/v2/context"
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/pr/shared"
...
func NewCmdCreate(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			To authorize, run "goctl auth refresh -s project".
		`, "`"),
		Example: heredoc.Doc(`
			$ goctl pr create --title "The bug is fixed" --body "Everything works again"
			$ goctl pr create --reviewer monalisa,hubot  --reviewer myorg/team-name
			$ goctl pr create --project "Roadmap"
			$ goctl pr create --base develop --head monalisa:feature
		`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Workaround: Due to the way this command is implemented, we need to manually check GOCTL_REPO.
			// Commands should use the standard BaseRepoOverride functionality to handle this behavior instead.
			if opts.RepoOverride == "" {
				opts.RepoOverride = os.Getenv("GOCTL_REPO")
			}
