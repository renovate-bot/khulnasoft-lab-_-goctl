...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ goctl pr list --author "@me"

			List only PRs with all of the given labels
			$ goctl pr list --label bug --label "priority 1"

			Filter PRs using search syntax
			$ goctl pr list --search "status:success review:required"

			Find a PR that introduced a given commit
			$ goctl pr list --search "<SHA>" --state merged
    	`),
		Aliases: []string{"ls"},
