...
// This file defines the 'goctl cs ssh' and 'goctl cs cp' subcommands.
...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/internal/codespaces"
	"github.com/khulnasoft-lab/goctl/v2/internal/codespaces/api"
	"github.com/khulnasoft-lab/goctl/v2/internal/codespaces/portforwarder"
	"github.com/khulnasoft-lab/goctl/v2/internal/codespaces/rpc"
...
func newSSHCmd(app *App) *cobra.Command {
	...
	sshCmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			run 'goctl cs ssh', select a codespace interactively, and connect.
			...
			if they were ordinary remote hosts (using 'ssh', not 'goctl cs ssh').
		...
		Example: heredoc.Doc(`
			...
			$ goctl codespace ssh --config > ~/.ssh/codespaces
...
func (a *App) printOpenSSHConfig(ctx context.Context, opts sshOptions) (err error) {
	...
	for result := range sshUsers {
		...
		type codespaceSSHConfig struct {
			...
			AutomaticIdentityFilePath string // path used for automatic private key `goctl cs ssh` would generate
		}
...
func newCpCmd(app *App) *cobra.Command {
	...
	cpCmd := &cobra.Command{
		...
		`, "`"),
		Example: heredoc.Doc(`
			$ goctl codespace cp -e README.md 'remote:/workspaces/$RepositoryName/'
			$ goctl codespace cp -e 'remote:~/*.go' ./gofiles/
			$ goctl codespace cp -e 'remote:/workspaces/myproj/go.{mod,sum}' ./gofiles/
			$ goctl codespace cp -e -- -F ~/.ssh/codespaces_config 'remote:~/*.go' ./gofiles/
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
