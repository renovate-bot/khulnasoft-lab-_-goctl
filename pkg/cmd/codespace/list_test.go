			...
			fields: fields{
				apiClient: &apiClientMock{
					ListCodespacesFunc: func(ctx context.Context, opts api.ListCodespacesOptions) ([]*api.Codespace, error) {
						...
						if opts.RepoName != "khulnasoft-lab/goctl" {
							return nil, fmt.Errorf("Expected repository name to be khulnasoft-lab/goctl. Got %s", opts.RepoName)
						}
						if opts.OrgName != "" {
