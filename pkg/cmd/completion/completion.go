...
func NewCmdCompletion(io *iostreams.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			When installing GitHub CLI througoctl a package manager, it's possible that
			...
			After, add this to your %[1]s~/.bash_profile%[1]s:
				...
				eval "$(goctl completion -s bash)"
			
			### zsh

			Generate a %[1]s_goctl%[1]s completion script and put it somewhere in your %[1]s$fpath%[1]s:

				goctl completion -s zsh > /usr/local/share/zsh/site-functions/_goctl
			...
			Generate a %[1]sgoctl.fish%[1]s completion script:

				goctl completion -s fish > ~/.config/fish/completions/goctl.fish
			...
			Add the line and save the file:
				...
				Invoke-Expression -Command $(goctl completion -s powershell | Out-String)
		`, "`"),
