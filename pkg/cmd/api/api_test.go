...
func Test_apiRun_cache(t *testing.T) {
	...
	t.Cleanup(func() {
		cacheDir := filepath.Join(os.TempDir(), "goctl-cli-cache")
...
func Test_fillPlaceholders(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "branch can't be filled when GOCTL_REPO is set",
			repoOverride: true,
			args: args{
				value: "repos/:owner/:repo/branches/:branch",
				opts: &ApiOptions{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.repoOverride {
				t.Setenv("GOCTL_REPO", "hubot/robot-uprising")
			}
