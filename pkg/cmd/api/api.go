...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/internal/ghinstance"
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/jsoncolor"
	"github.com/khulnasoft-lab/go-goctl/v2/pkg/jq"
	"github.com/khulnasoft-lab/go-goctl/v2/pkg/template"
...
func NewCmdApi(f *cmdutil.Factory, runF func(*ApiOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			directory or the repository specified in the GOCTL_REPO environment variable.
		...
		Example: heredoc.Doc(`
			...
			$ goctl api repos/{owner}/{repo}/releases

			# post an issue comment
			$ goctl api repos/{owner}/{repo}/issues/123/comments -f body='Hi from CLI'

			# post nested parameter read from a file
			$ goctl api gists -F 'files[myfile.txt][content]=@myfile.txt'

			# add parameters to a GET request
			$ goctl api -X GET search/issues -f q='repo:khulnasoft-lab/goctl is:open remote'

			# set a custom HTTP header
			$ goctl api -H 'Accept: application/vnd.github.v3.raw+json' ...

			# opt into GitHub API previews
			$ goctl api --preview baptiste,nebula ...

			# print only specific fields from the response
			$ goctl api repos/{owner}/{repo}/issues --jq '.[].title'

			# use a template for the output
			$ goctl api repos/{owner}/{repo}/issues --template \
			  '{{range .}}{{.title}} ({{.labels | pluck "name" | join ", " | color "yellow"}}){{"\n"}}{{end}}'

			# list releases with GraphQL
			$ goctl api graphql -F owner='{owner}' -F name='{repo}' -f query='
			...
			$ goctl api graphql --paginate -f query='
		...
		Annotations: map[string]string{
			"help:environment": heredoc.Doc(`
				GOCTL_TOKEN, GITHUB_TOKEN (in order of precedence): an authentication token for
				github.com API requests.

				GOCTL_ENTERPRISE_TOKEN, GITHUB_ENTERPRISE_TOKEN (in order of precedence): an
				authentication token for API requests to GitHub Enterprise.

				GOCTL_HOST: make the request to a GitHub host other than github.com.
			`),
		},
	...
	cmd.Flags().StringVarP(&opts.Template, "template", "t", "", "Format JSON output using a Go template; see \"goctl help formatting\"")
