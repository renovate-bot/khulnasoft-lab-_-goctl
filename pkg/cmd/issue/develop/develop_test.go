...
func TestNewCmdDevelop(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "issue url",
			input: "https://github.com/khulnasoft-lab/goctl/issues/1",
			output: DevelopOptions{
				IssueSelector: "https://github.com/khulnasoft-lab/goctl/issues/1",
			},
		},
		{
		...
		},
		{
			...
			input: "1 --issue-repo khulnasoft-lab/goctl",
			output: DevelopOptions{
				IssueSelector: "1",
			},
...
func TestDevelopRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: "the `goctl issue develop` command is not currently available",
		},
		{
		...
		},
		{
			...
			opts: &DevelopOptions{
				IssueSelector: "https://github.com/khulnasoft-lab/goctl/issues/42",
				List:          true,
			},
			httpStubs: func(reg *httpmock.Registry, t *testing.T) {
				reg.Register(
		...
		},
		{
			...
			opts: &DevelopOptions{
				IssueSelector: "https://github.com/khulnasoft-lab/goctl/issues/123",
			},
			httpStubs: func(reg *httpmock.Registry, t *testing.T) {
				reg.Register(
					httpmock.GraphQL(`query LinkedBranchFeature\b`),
			...
			expectedOut: "github.com/khulnasoft-lab/goctl/tree/my-issue-1\n",
		},
		{
