...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/context"
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/issue/shared"
...
func NewCmdDevelop(f *cmdutil.Factory, runF func(*DevelopOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ goctl issue develop --list 123

			# List branches for issue 123 in repo khulnasoft-lab/goctl
			$ goctl issue develop --list --repo khulnasoft-lab/goctl 123

			# Create a branch for issue 123 based on the my-feature branch
			$ goctl issue develop 123 --base my-feature

			# Create a branch for issue 123 and checkout it out
			$ goctl issue develop 123 --checkout

			# Create a branch in repo monalisa/cli for issue 123 in repo khulnasoft-lab/goctl
			$ goctl issue develop 123 --repo khulnasoft-lab/goctl --branch-repo monalisa/cli
		`),
