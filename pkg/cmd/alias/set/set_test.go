...
func TestSetRun(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			opts: &SetOptions{
				Name:      "igrep",
				Expansion: "!goctl issue list | grep",
			},
			wantExpansion: "!goctl issue list | grep",
			wantStderr:    "- Creating alias for igrep: !goctl issue list | grep\n✓ Added alias igrep\n",
		},
		{
		...
		},
		{
			...
			wantErrMsg: "X Could not create alias foo: expansion does not correspond to a goctl command, extension, or alias",
