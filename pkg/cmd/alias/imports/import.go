...
func NewCmdImport(f *cmdutil.Factory, runF func(*ImportOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			The output from the goctl command "alias list" can be used to produce a YAML file
			containing your aliases, which you can use to import them from one machine to
			another. Run "goctl help alias list" to learn more.
		`),
		Example: heredoc.Doc(`
			# Import aliases from a file
			$ goctl alias import aliases.yml
...
func importRun(opts *ImportOptions) error {
	...
	for _, alias := range getSortedKeys(aliasMap) {
		...
		if !opts.validAliasName(alias) {
			if !existingAlias {
				msg.WriteString(
					fmt.Sprintf("%s Could not import alias %s: already a goctl command or extension\n",
						cs.FailureIcon(),
						cs.Bold(alias),
					),
				)
		...
		if !opts.validAliasExpansion(expansion) {
			msg.WriteString(
				fmt.Sprintf("%s Could not import alias %s: expansion does not correspond to a goctl command, extension, or alias\n",
					cs.FailureIcon(),
					cs.Bold(alias),
				),
			)
