	...
	tests := []struct {
	...
	}{
		{
			...
			fileContents: heredoc.Doc(`
                co: pr checkout
                igrep: '!goctl issue list --label="$1" | grep "$2"'
            `),
		...
		},
		{
			...
			wantStderr: strings.Join(
				[]string{
					importFileMsg,
					"X Could not import alias api: already a goctl command or extension",
					"X Could not import alias issue: already a goctl command or extension",
					"X Could not import alias pr: already a goctl command or extension\n\n",
				},
				"\n",
			),
		},
		{
			...
			wantStderr: strings.Join(
				[]string{
					importFileMsg,
					"X Could not import alias alias1: expansion does not correspond to a goctl command, extension, or alias",
					"X Could not import alias alias2: expansion does not correspond to a goctl command, extension, or alias\n\n",
				},
				"\n",
			),
		},
