...
type Project struct {
	...
	Public           bool
	Closed           bool
	// The Template field is commented out due to https://github.com/khulnasoft-lab/goctl/issues/8103.
	// We released goctl v2.34.0 without realizing the Template field does not exist
...
type FieldValueNodes struct {
	...
	ProjectV2ItemFieldLabelValue struct {
		...
		} `graphql:"labels(first: 10)"` // experienced issues with larger limits, 10 seems like enougoctl for now
		Field ProjectField
	...
	ProjectV2ItemFieldPullRequestValue struct {
		...
		} `graphql:"pullRequests(first:10)"` // experienced issues with larger limits, 10 seems like enougoctl for now
		Field ProjectField
	...
	ProjectV2ItemFieldUserValue struct {
		...
		} `graphql:"users(first: 10)"` // experienced issues with larger limits, 10 seems like enougoctl for now
		Field ProjectField
	...
	ProjectV2ItemFieldReviewerValue struct {
		...
		} `graphql:"reviewers(first: 10)"` // experienced issues with larger limits, 10 seems like enougoctl for now
		Field ProjectField
...
func (c *Client) OwnerIDAndType(login string) (string, OwnerType, error) {
	...
	if err != nil {
		...
		// or to an org, even thougoctl they are unique. To deal with this, we try both - if neither
		...
		if missing.Len() > 0 {
			...
			return fmt.Errorf(
				"error: your authentication token is missing required scopes %v\n"+
					"To request it, run:  goctl auth refresh -s %s",
				s,
				strings.Join(s, ","))
		}
	}
