...
import (
	...
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/project/shared/client"
	"github.com/khulnasoft-lab/goctl/v2/pkg/cmd/project/shared/format"
...
func NewCmdCreateField(f *cmdutil.Factory, runF func(config createFieldConfig) error) *cobra.Command {
	...
	createFieldCmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			goctl project field-create 1 --owner "@me" --name "new field" --data-type "text"

			# create a field with three options to select from for owner monalisa
			goctl project field-create 1 --owner monalisa --name "new field" --data-type "SINGLE_SELECT" --single-select-options "one,two,three"
		`),
