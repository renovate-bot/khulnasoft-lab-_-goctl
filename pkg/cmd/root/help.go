...
func rootHelpFunc(f *cmdutil.Factory, command *cobra.Command, args []string) {
	...
	if longText != "" && command.LocalFlags().Lookup("jq") != nil {
		longText = strings.TrimRight(longText, "\n") +
			"\n\nFor more information about output formatting flags, see `goctl help formatting`."
	}
...
Use 'goctl <command> <subcommand> --help' for more information about a command.
...
func authHelp() string {
	if os.Getenv("GITHUB_ACTIONS") == "true" {
		return heredoc.Doc(`
			gh: To use GitHub CLI in a GitHub Actions workflow, set the GOCTL_TOKEN environment variable. Example:
			  env:
			    GOCTL_TOKEN: ${{ github.token }}
		`)
	}

	if os.Getenv("CI") != "" {
		return heredoc.Doc(`
			gh: To use GitHub CLI in automation, set the GOCTL_TOKEN environment variable.
		`)
	}

	return heredoc.Doc(`
		To get started with GitHub CLI, please run:  goctl auth login
		Alternatively, populate the GOCTL_TOKEN environment variable with a GitHub API authentication token.
	`)
}
