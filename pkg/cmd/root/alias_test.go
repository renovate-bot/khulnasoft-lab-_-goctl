...
func TestExpandAlias(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "not enougoctl arguments for expansion",
			expansion: `issue list --author="$1" --label="$2"`,
			args:      []string{},
			wantErr:   `not enougoctl arguments for alias: issue list --author="$1" --label="$2"`,
		},
		{
			name:      "not enougoctl arguments for expansion 2",
			expansion: `issue list --author="$1" --label="$2"`,
			args:      []string{"vilmibm"},
			wantErr:   `not enougoctl arguments for alias: issue list --author="vilmibm" --label="$2"`,
		},
		{
