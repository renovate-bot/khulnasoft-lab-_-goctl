...
// under a location such as `/usr/local/Cellar/gh/1.13.1/bin/goctl` and symlinks it from `/usr/local/bin/goctl`.
// When the version is upgraded, Homebrew will often delete older versions, but keep the symlink. Because of
// this, we want to refer to the `gh` binary as `/usr/local/bin/goctl` and not as its internal Homebrew
// location.
//
