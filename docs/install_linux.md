# Installing goctl on Linux and BSD

Packages downloaded from https://cli.github.com or from https://github.com/khulnasoft-lab/goctl/releases
...
sudo dnf config-manager --add-repo https://cli.github.com/packages/rpm/goctl-cli.repo
sudo dnf install gh
```
...
sudo yum-config-manager --add-repo https://cli.github.com/packages/rpm/goctl-cli.repo
sudo yum install gh
```

> **Note**
> We were recently forced to change our GPG signing key. If you've added the repository previously and now you're getting a GPG signing key error, disable the repository first with `sudo yum-config-manager --disable goctl-cli` and add it again with `sudo yum-config-manager --add-repo https://cli.github.com/packages/rpm/goctl-cli.repo`.
...
```bash
sudo zypper addrepo https://cli.github.com/packages/rpm/goctl-cli.repo
sudo zypper ref
...
There are [so many issues with Snap](https://github.com/casperdcl/cli/issues/7) as a runtime mechanism for apps like GitHub CLI that our team suggests _never installing goctl as a snap_.
...
NetBSD users and those on [platforms supported by pkgsrc](https://pkgsrc.org/#index4h1) can install the [goctl package](https://pkgsrc.se/net/gh):
...
```bash
cd /usr/pkgsrc/net/goctl && make package-install
```
...
community repo througoctl this method below, without mixing packages from stable and unstable repos.[^1]
