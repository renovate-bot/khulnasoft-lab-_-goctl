...
2. Clone this repository
   ...
   ```sh
   $ git clone https://github.com/khulnasoft-lab/goctl.git goctl-cli
   $ cd goctl-cli
   ```
...
4. Run `goctl version` to check if it worked.

   #### Windows
   Run `bin\goctl version` to check if it worked.
...
$ GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=0 make clean bin/goctl
```
```pwsh
# on Windows, pass environment variables as arguments to the build script:
> go run script\build.go clean bin\goctl GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=0
```
