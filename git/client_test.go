...
func TestClientSetRemoteResolution(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantCmdArgs: `path/to/git config --add remote.origin.goctl-resolved base`,
		},
		{
			name:          "git error",
			cmdExitStatus: 1,
...
func TestClientUnsetRemoteResolution(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantCmdArgs: `path/to/git config --unset remote.origin.goctl-resolved`,
		},
		{
			name:          "git error",
			cmdExitStatus: 1,
...
func TestClientClone(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "clone",
			wantCmdArgs: `path/to/git -c credential.helper= -c credential.helper=!"gh" auth git-credential clone github.com/khulnasoft-lab/goctl`,
			wantTarget:  "cli",
		},
		{
			name:        "accepts command modifiers",
			mods:        []CommandModifier{WithRepoDir("/path/to/repo")},
			wantCmdArgs: `path/to/git -C /path/to/repo -c credential.helper= -c credential.helper=!"gh" auth git-credential clone github.com/khulnasoft-lab/goctl`,
			wantTarget:  "cli",
		},
		{
			name:          "git error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			target, err := client.Clone(context.Background(), "github.com/khulnasoft-lab/goctl", []string{}, tt.mods...)
			assert.Equal(t, tt.wantCmdArgs, strings.Join(cmd.Args[3:], " "))
			if tt.wantErrorMsg == "" {
				assert.NoError(t, err)
			} else {
...
func TestHelperProcess(t *testing.T) {
	...
	if err := func(args []string) error {
		fmt.Fprint(os.Stdout, os.Getenv("GOCTL_HELPER_PROCESS_STDOUT"))
		exitStatus := os.Getenv("GOCTL_HELPER_PROCESS_EXIT_STATUS")
		if exitStatus != "0" {
			return errors.New("error")
		}
		return nil
	}(os.Args[3:]); err != nil {
		fmt.Fprint(os.Stderr, os.Getenv("GOCTL_HELPER_PROCESS_STDERR"))
		exitStatus := os.Getenv("GOCTL_HELPER_PROCESS_EXIT_STATUS")
...
func createCommandContext(t *testing.T, exitStatus int, stdout, stderr string) (*exec.Cmd, commandCtx) {
	...
	cmd.Env = []string{
		"GOCTL_WANT_HELPER_PROCESS=1",
		fmt.Sprintf("GOCTL_HELPER_PROCESS_STDOUT=%s", stdout),
		fmt.Sprintf("GOCTL_HELPER_PROCESS_STDERR=%s", stderr),
		fmt.Sprintf("GOCTL_HELPER_PROCESS_EXIT_STATUS=%v", exitStatus),
	}
