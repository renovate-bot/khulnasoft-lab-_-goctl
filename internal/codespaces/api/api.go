...
// ListCodespaceFields is the list of exportable fields for a codespace when using the `goctl cs list` command.
...
// ViewCodespaceFields is the list of exportable fields for a codespace when using the `goctl cs view` command.
...
func (a *API) EditCodespace(ctx context.Context, codespaceName string, params *EditCodespaceParams) (*Codespace, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if resp.StatusCode == http.StatusUnprocessableEntity {
			...
			// this fall througoctl to the normal api.HandleHTTPError flow
