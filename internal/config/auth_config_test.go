...
func newTestAuthConfig(t *testing.T) *AuthConfig {
	...
	// we can use GOCTL_CONFIG_DIR env var to ensure the tests remain isolated.
	StubWriteConfig(t)
...
func TestTokenStoredInConfig(t *testing.T) {
	...
	// https://github.com/khulnasoft-lab/go-goctl/issues/94
	require.Equal(t, "test-token", token)
...
func TestTokenStoredInEnv(t *testing.T) {
	...
	authCfg := newTestAuthConfig(t)
	t.Setenv("GOCTL_TOKEN", "test-token")
	...
	require.Equal(t, "test-token", token)
	require.Equal(t, "GOCTL_TOKEN", source)
}
...
func TestHasEnvTokenWithEnvToken(t *testing.T) {
	...
	// Note that any valid env var for tokens will do, not just GOCTL_ENTERPRISE_TOKEN
	authCfg := newTestAuthConfig(t)
	t.Setenv("GOCTL_ENTERPRISE_TOKEN", "test-token")
...
func TestHostsIncludesEnvVar(t *testing.T) {
	// Given the GOCTL_HOST env var is set
	authCfg := newTestAuthConfig(t)
	t.Setenv("GOCTL_HOST", "ghe.io")
...
func TestDefaultHostFromEnvVar(t *testing.T) {
	...
	require.Equal(t, "GOCTL_HOST", source)
}
